...
const SetUnread = (chatRooms: ChatRoom[], chatRoomId: string, unreadCount: number) => {
    const found = chatRooms.find((val) => val.chatRoomId === chatRoomId);
    const foundIdx = chatRooms.findIndex((val) => val.chatRoomId === chatRoomId);
    if (found === undefined) {
        return chatRooms;
    } else {
...
const UpdateChatRoom = (chatRooms: ChatRoom[], message: resMessage, activeRoom?: ChatRoom) => {
    const msgInChatRoom = chatRooms.find((room) => room.chatRoomId === message.chatRoomId);
    console.log("de", msgInChatRoom);
    if( msgInChatRoom === undefined ){
        // TODO: add chatroom
        return;
    }
    else if( activeRoom === undefined || msgInChatRoom.chatRoomId !== activeRoom.chatRoomId ){
        msgInChatRoom.lastMessage = message.content;
        msgInChatRoom.unreadCount = msgInChatRoom.unreadCount === undefined ? 0 : msgInChatRoom.unreadCount + 1;
...
const UpdateOnConnected = (chatRooms: ChatRoom[], resChatRooms: ResChatRoom[]) => {
    ...
    const ret: ChatRoom[] = resChatRooms.map((room) => {
        const messageLength = new TypedStorage<IMessageStorage>(`sentMessages-${room.chatRoomId}`).get()?.messages.length ?? 0;
        return {
            ...
            unreadCount: room.messageSeq - messageLength - 1,
        }
    })
...
export const useChatRoomStore = create<ChatRoomStore>((set) => ({
    chatRooms: JSON.parse(safeLocalStorage.get('chatRooms') ?? '[]') as ChatRoom[],
    activeRoom: undefined,
    tempId: undefined,
    setTempId: (id) => set((state) => ({tempId: id})),
    socket: io('ws://localhost:8080', {withCredentials: true, path: "/api/chat", autoConnect: false}),
    removeSocket: () => set((state) => ({socket: undefined})),
    setActiveRoom: (chatRoomId) => set((state) => ({activeRoom: SetActiveRoom(state.chatRooms, chatRoomId)})),
    setChatRooms: (chatRooms) => set(() => ({chatRooms: chatRooms})),
    removeChatRoom: (chatRoomId) => set((state) => ({chatRooms: RemoveChatRoom(state.chatRooms, chatRoomId)})),
    updateChatRoom: (message) => set((state) => ({chatRooms: UpdateChatRoom(state.chatRooms, message, state.activeRoom)})),
    updateOnConnect: (resChatRooms) => set((state) => ({chatRooms: UpdateOnConnected(state.chatRooms, resChatRooms)})),
    setUnread: (chatRoomId, undreadCount) => set((state) => ({chatRooms: SetUnread(state.chatRooms, chatRoomId, undreadCount)})),
}))
